...
for train_index, test_index in cv.split(cuiPairs):
    ...
    f_tr = open('TransE/TransE'+str(f)+'/training.csv', 'w')
    f_tes= open('TransE/TransE'+str(f)+'/testing.csv', 'w')
    ...
    for j in X_trainPairs.keys():
        ...
        if interaction==1:
           ...
           #check0 = input("PAUSE.")
           insert = run_query("""
               MATCH (s:Annotation),(t:Annotation) WHERE s.id = '"""+drCUI+"""' AND t.id = '"""+tarCUI+"""'
    ...
    tf1.to_path_binary('dictionaryU.pt')
    print('Done! ')
    ...
    for j in X_trainPairs.keys():
        ...
        if interaction==1: #positive train pair must be deleted from KG  after embedding creation...
            ...
            tarCUI= cuiList[1]
            delete = run_query("""
                MATCH (s:Annotation)-[r:INTERACTION]->(t:Annotation) WHERE s.id = '"""+drCUI+"""' AND t.id = '"""+tarCUI+"""'
                DELETE r
                """)
    f_tr.close()
    #check1 = input("PAUSE.")
