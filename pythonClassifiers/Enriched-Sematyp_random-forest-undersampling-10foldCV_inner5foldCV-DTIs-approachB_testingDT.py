...
def tenFoldCV(X, y, cv, model, header):
   ...
   for train_index, test_index in cv.split(X, y):
         ...
         strategy = [0,0.2]#runInnerCV(X_train, y_train)
         algo = strategy[0]
         ratio= strategy[1]
...
def runInnerCV(inner_X, inner_y):
   ...
   #ratios=[0.11, 0.15, 0.17, 0.2, 0.25]
...
def main():
   data = pd.read_csv("DTI-enriched_sematyp_featuresExtended.csv")
   ...
   export_graphviz(dt, out_file=dot_data,  
                ...
                special_characters=True,feature_names = header_important[0],class_names=['NO','YES'])
   graph = pydotplus.graph_from_dot_data(dot_data.getvalue())  
   graph.write_png('decisiontree_approx.png')
