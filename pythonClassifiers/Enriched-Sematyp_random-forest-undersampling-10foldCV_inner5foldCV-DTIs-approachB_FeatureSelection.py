...
def tenFoldCV(X, y, cv, model, z, header):
   ...
   for train_index, test_index in cv.split(X):
         ...
         ####FOT ADDITION for False negatives investigation...
         # open csv file to write FN,TP,TN cases...
         if i==0:
            w_fn = open('/home/fot/Desktop/fns.csv', 'w')
            w_tp = open('/home/fot/Desktop/tps.csv', 'w')
            w_tn = open('/home/fot/Desktop/tns.csv', 'w')
         else:
            w_fn= open('/home/fot/Desktop/fns.csv', 'a')
            w_tp= open('/home/fot/Desktop/tps.csv', 'a')
            w_tn = open('/home/fot/Desktop/tns.csv', 'a')
         writerTP = csv.writer(w_tp)
         writerFN = csv.writer(w_fn)
         writerTN = csv.writer(w_tn)
         if i==0:
             
             head= np.array(['Drug_Target'])
             head = np.concatenate((head, header[0]), axis=0)
             writerTP.writerow(head)
             writerFN.writerow(head)
             writerTN.writerow(head)
         fns = ret_FN(y_test, y_pred)
         tps = ret_TP(y_test, y_pred)
         tns = ret_TN(y_test, y_pred)
         for f in fns:
            row=[]
            row.append(z_test.iloc[f])
            for num in X_test[f]:
               row.append(num)
            writerFN.writerow(row)
         for f in tps:
            row=[]
            row.append(z_test.iloc[f])
            for num in X_test[f]:
               row.append(num)
            writerTP.writerow(row)
         for f in tns:
            row=[]
            row.append(z_test.iloc[f])
            for num in X_test[f]:
               row.append(num)
            writerTN.writerow(row)      
         w_fn.close()
         w_tp.close()
         w_tn.close()
         ####END - FOT ADDITION
...
def runInnerCV(inner_X, inner_y):
   ...
   strategy = 0
   maxf1=0
   ratios=[0.02, 0.03, 0.05, 0.075, 0.1]
...
def main():   
   #data = pd.read_csv("/home/fot/workspace/python/approachb-DTI-SemaTyp-features-aggregated.csv")
...
##   X_important= rfe.transform(X)
   ...
   sfm = SelectFromModel(model,threshold=0.004) #prefit=True)
   ...
   #header_important= sfm.transform(header)
   #Χ_important= sfm.transform(Χ)
   print('f1score with all features: ',f1)
   
   # evaluate model
   print('10-fold CV evaluate model...')
