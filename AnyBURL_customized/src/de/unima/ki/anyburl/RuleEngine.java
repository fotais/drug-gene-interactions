...
public class RuleEngine {
	...
	*/
	...
	public static double predictMax(TripleSet testSet, TripleSet trainingSet, TripleSet validationSet, int k, HashMap<String, ArrayList<Rule>> relation2Rules4Prediction, Triple triple) {
		ScoreTree kTree = new ScoreTree();
		//LinkedHashMap<String, Double> kTailCandidates = predictTripleScore(testSet, trainingSet, validationSet, k, relation2Rules4Prediction, triple, false, kTree);
		...
		//LinkedHashMap<String, Double> kHeadCandidates = predictTripleScore(testSet, trainingSet, validationSet, k, relation2Rules4Prediction, triple, true, kTree);
		...
		if (kHeadCandidates.containsKey(head))
			headscore=kHeadCandidates.get(head);
		if (kTailCandidates.containsKey(tail))
			tailscore= kTailCandidates.get(tail);
	...
	*/
	...
	public static LinkedHashMap<String, Double> predictTripleScore(TripleSet testSet, TripleSet trainingSet, TripleSet validationSet, int k, HashMap<String, ArrayList<Rule>> relation2Rules, Triple triple, boolean predictHeadNotTail, ScoreTree kTree) {
...
//			System.out.println("Predicting heads for tail: " +head+"-"+tail);
//		else
//			System.out.println("Predicting tails for head: " +head+"-"+tail);

		if (relation2Rules.containsKey(relation)) {
			ArrayList<Rule> relevantRules = relation2Rules.get(relation);
			Set<String> candidates = new HashSet<String>();
			Set<String> fCandidates = new HashSet<String>();
			//FOT - need to check whether head/tail confirm each rule, else leave fcandidates empty...
			for (Rule rule : relevantRules) {
				if (predictHeadNotTail) {
					candidates = rule.computeHeadResults(tail, trainingSet);
					if (!candidates.contains(head))
						continue;
					else
						fCandidates.add(head);
				}
				else {
					candidates = rule.computeTailResults(head, trainingSet);
					if (!candidates.contains(tail))
						continue;
					else
						fCandidates.add(tail);
				}
				//FOT: fCandidates must include only the candidates that confirm this rule!!!
				if (!kTree.fine()) {
					if (fCandidates.size() > 0) {
						if (Settings.PATH_EXPLANATION != null) kTree.addValues(rule.getAppliedConfidence(), fCandidates, rule);
						else kTree.addValues(rule.getAppliedConfidence(), fCandidates, null);
...
//			System.out.println("candidate: "+candidate+" value="+kCandidates.get(candidate));
	...
	public static void predictMaxPlusSpecial(TripleSet testSet, TripleSet trainingSet, TripleSet validationSet, HashMap<String, ArrayList<Rule>> relation2Rules, Triple triple,  HashMap<String, HashMap<Rule,Integer>> relation2RulesIndex) {
		...
		System.out.println("%%%%%%%%555tail="+tail+", kTailCandidates size="+kTailCandidates.size());
		if (kTailCandidates.containsKey(head))
			System.out.println("*******************kTailCandidates contains "+head);
